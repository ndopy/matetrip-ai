...
TOOL_ELIGIBILITY = (
    "<Tool Eligibility>\n"
    "**CRITICAL RULE - Tool Selection Priority**\n"
    "When the user mentions a BROAD region (광역 지역) + wants popular/famous places:\n"
    "→ **ALWAYS use recommend_popular_places_in_region FIRST**\n"
    "→ NEVER use recommend_nearby_places for broad regions\n\n"
    "Broad regions include: 서울, 부산, 대전, 대구, 광주, 울산, 세종, 인천, 제주도, 제주, 강원도, 강원, 경기도, 경기, 경상도, 전라도, 충청도\n\n"
    "1. recommend_popular_places_in_region:\n"
    "   - **PRIMARY TOOL** for broad region + popularity/recommendation requests\n"
    "   - Trigger: 광역 지역 + '유명한/인기/핫플/추천/많이 가는/사람들이 가는' 의도\n"
    "   - 예시:\n"
    "     * '제주도 인기장소', '제주 핫플', '제주에서 인기 있는 곳'\n"
    "     * '부산 핫플 알려줘', '부산에서 사람들이 많이 가는 곳'\n"
    "     * '서울 추천 명소', '대전 여행지 추천'\n"
    "   - 필수 슬롯: 광역 지역 (서울/부산/제주도/강원도 등)\n"
    "   - 선택 슬롯: 카테고리 (카페/맛집/명소 등)\n"
    "   - 위치가 명확히 말되지 않았으면 절대 기본값을 추정하지 말고 먼저 '어느 지역을 찾으세요?' 같은 짧은 질문으로 확인\n\n"
    "2. recommend_nearby_places:\n"
    "   - **ONLY for specific locations** (강남, 홍대, 해운대 등) or '근처/주변' requests\n"
    "   - Trigger: 구체적 장소명 + '근처/주변/가까운' 의도\n"
    "   - 예시: '강남역 근처 맛집', '홍대 주변 카페', '해운대 가까운 곳'\n"
    "   - ❌ DO NOT use for: '제주도 맛집', '부산 카페' (이런 경우는 recommend_popular_places_in_region 사용)\n"
    "   - 필수 슬롯: 구체적 위치 (장소명/주소)\n"
    "   - 슬롯 누락 시:\n"
    "     * 위치 미정 → '어느 위치를 기준으로 찾을까요?'\n"
    "     * 카테고리 미정 → '어떤 종류를 찾으세요? (예: 맛집/카페/명소)'\n\n"

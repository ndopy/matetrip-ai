...
# TreadPoolExecutor는 동기 방식만 인자로 받기 때문에 이렇게 함
...
def handle_travel_route_notification(state: AgentState, content: Any) -> None:
    ...
    try:
        ...
        if not content.get("success", False):
            logger.warning(
                f"[travel_route] Tool execution failed: {content.get('error')}"
            )
            return
        ...
        logger.error(f"[travel_route] Error handling route notification: {e}")

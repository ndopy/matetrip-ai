...
def test_table_schema():
    ...
    print("\n[places 테이블]")
    places_columns = {
        col["name"]: col["type"] for col in inspector.get_columns("places")
    }
    ...
    print("\n[place_review 테이블]")
    review_columns = {
        col["name"]: col["type"] for col in inspector.get_columns("place_review")
    }
    ...
    try:
        with SessionLocal() as db:
            ...
            if place:
                ...
                print(
                    f"  - embedding: {'있음' if place.embedding is not None else '없음'}"
                )
            ...
            if review:
                ...
                print(
                    f"  - embedding: {'있음' if review.embedding is not None else '없음'}"
                )
            else:
    ...
    try:
        with SessionLocal() as db:
            ...
            reviews_with_embedding = (
                ...
                .filter(
                    PlaceReview.embedding.isnot(None), PlaceReview.is_deleted == False
                )
                .count()
            )

            # 장소 임베딩이 있는 개수
            places_with_embedding = (
                db.query(Place).filter(Place.embedding.isnot(None)).count()
            )
            ...
            # 임베딩이 필요한 장소 개수
            places_need_embedding = (
                db.query(Place).filter(Place.embedding.is_(None)).count()
            )

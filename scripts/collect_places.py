...
#         min_popularity_score: int = 5,  # 최소 블로그 검색 결과 개수
#     ):
#         self.db = db
...
#         self.filtered_count = 0  # 인기도 부족으로 필터링된 개수
#         self.error_count = 0
...
#         logger.info(f"인기도 필터: 최소 {self.min_popularity_score}개 이상의 블로그 글")
...
#         logger.info(f"- 인기도 부족으로 필터링: {self.filtered_count}개")
...
#         # 인기도 체크: 네이버 블로그 검색 결과 개수로 판단
#         popularity_score = self.naver_service.get_place_popularity_score(
#             place_data["title"], place_data["address"]
#         )

#         if popularity_score < self.min_popularity_score:
#             self.filtered_count += 1
#             logger.info(
#                 f"    ✗ 필터링: {place_data['title']} "
#                 f"(블로그 글: {popularity_score}개 < {self.min_popularity_score}개)"
#             )
#             return
